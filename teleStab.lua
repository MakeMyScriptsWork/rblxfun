...
local function forceTeleport(targetRoot, duration)
    ...
    connection = RunService.RenderStepped:Connect(function()
        ...
        if tick() - startTime >= duration then
            ...
            print("Teleport connection disconnected.")
        end
    end)
    connections[#connections + 1] = connection
    while tick() - startTime < duration and connection.Connected do
...
local function performAttack(dropdown)
    ...
    if not success then
        print("Error in attack sequence: " .. tostring(err))
    end
...
local success, guiError = pcall(function()
    ...
    dropdownButton.Position = UDim2.new(0.1, 0, 0.1, 0)
    ...
    -- Button connections (moved earlier to avoid crash)
    local success, buttonError
    success, buttonError = pcall(function()
        connections[#connections + 1] = button.MouseButton1Click:Connect(function()
            print("Attack button clicked!")
            performAttack(dropdownFrame)
        end)
        print("Attack button connection established.")
    end)
    if not success then
        print("Attack button connection failed: " .. tostring(buttonError))
    end
    ...
    success, buttonError = pcall(function()
        ...
        print("Dropdown button connection established.")
    end)
    if not success then
        print("Dropdown button connection failed: " .. tostring(buttonError))
    end
    ...
    success, buttonError = pcall(function()
        ...
        print("Close button connection established.")
    end)
    if not success then
        print("Close button connection failed: " .. tostring(buttonError))
    end
    ...
    -- Simplified manual drag handler
        ...
        if input.UserInputType == Enum.UserInputType.MouseButton1 and not dropdownFrame.Visible then
            ...
            if mousePos.X >= framePos.X and mousePos.X <= framePos.X + frameSize.X and
               mousePos.Y >= framePos.Y and mousePos.Y <= framePos.Y + frameSize.Y then
                ...
                print("Started dragging GUI, initial position: " .. tostring(dragStartPos))
            end
        end
    end)
    ...
    connections[#connections + 1] = UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging = false
            print("Stopped dragging GUI, final position: " .. tostring(frame.Position))
        end
    end)
    ...
    connections[#connections + 1] = UserInputService.InputChanged:Connect(function(input)
        if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
            local mousePos = input.Position
            local delta = mousePos - dragStartMousePos
            ...
            print("Dragging GUI, new position: " .. tostring(frame.Position))
        end
    end)
